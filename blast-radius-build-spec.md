...
**JacHacks SF · Track: fintech / open · Subprizes targeted: best use of Jac, best use of JacHammer, best defense application**
...
Not a visualization. Three actions with outputs someone uses.
...
Vendors add subprocessors quietly. Most DPAs give you a contractual right to notice and often a right to object — a right nobody exercises, because nobody notices. A scheduled walker re-crawls weekly and diffs the graph: *"Notion added a new subprocessor on Nov 3. You have 30 days to object under §7.2."*
...
Be honest in the UI that this finds *externally visible* vendors. "I typed your domain and found nine of your vendors without you telling me anything" is a strong claim precisely because it's a true one.
...
### 4.3 Discovery fallback for the long tail

No standard path exists. In priority order:

1. **`/sitemap.xml`**, grep for `subprocess`, `sub-process`, `dpa`. One request, better hit rate than guessing.
2. **`trust.<domain>`** — a very common CNAME, since most of these companies run their trust center on Vanta, SafeBase, or Drata.
3. **Path guessing**, ~15 candidates in parallel:
   `/subprocessors` `/sub-processors` `/legal/subprocessors` `/legal/sub-processors` `/trust/subprocessors` `/security/subprocessors` `/privacy/subprocessors` `/dpa` `/legal/dpa` `/legal/data-processing-addendum`
4. Scoped web search, last resort.
...
Pitch line: *"The graph only converges because the language does the entity resolution. That's not a prompt I wrote — it's a type signature."*
...
`[prov <-:Subprocesses:<-]` is the whole product in one line. That query — *who transitively depends on this* — is the thing a spreadsheet structurally cannot answer and a graph answers for free.
...
The graph is what makes the chokepoint number land emotionally. But the *default* version of this looks bad, and the specific choices below are the difference between "sick" and "noisy."
...
This is the single biggest visual win in the project. Pin each node's horizontal position by its tier and let the simulation resolve vertical placement only. Now it reads like a Sankey diagram, and the convergence becomes *structurally* visible: many vendors on the left funneling into a few fat nodes on the right. The picture argues your thesis before you open your mouth.
...
On *Simulate outage*: desaturate the entire graph to ~15% opacity, light the failed provider red, then animate the failure spreading **backwards** along inbound edges — provider → vendors → your features. Roughly 2 seconds, staggered per hop.

It is a literal rendering of `[prov <-:Subprocesses:<-]`. Say that out loud while it plays: *"that animation is one line of Jac."*
...
| Time | Beat |
|---|---|
| 0:00 | "Your vendors have vendors. You've never seen that list." Ask a judge to name their company. |
| 0:20 | Type their domain. Detection fires. 9 vendors appear from DNS, headers, and scripts, unprompted. |
| 0:40 | They add 3 internal tools in the text box. Crawl starts; nodes stream in live. |
| 1:10 | **The reveal.** 34 vendors resolve to 11 real providers. Chokepoint ranking on screen. "71% of your stack terminates at one provider. Your redundancy is a myth." |
| 1:40 | Click *Simulate outage*. Subgraph goes red. Affected features list. Drafted status post. |
| 2:05 | Action 1: export the questionnaire answer. Action 2: the diff monitor catching a silently-added subprocessor last week. |
| 2:30 | Open the JacHammer URL on their phone. Show the Jac admin portal's built-in graph visualization and auto-generated Swagger. "One file. `jac start --scale`. No Dockerfile, no migrations, no tenancy code." |
| 2:50 | Log in as a second user. Empty graph. "Per-user root isolation. Zero lines of multi-tenancy code — that's the language, not us." |
...
**Best use of Jac.** Say the words *object-spatial*, *meaning-typed*, *scale-invariant* — Jaseci judges are listening for exactly those and most teams won't say them.
...
- `by llm()` for structured extraction *and* entity resolution — the graph doesn't converge without the latter.
...
**Best defense application.** Second tab, *same walkers*, different data source: public DoD prime → sub → tier-3 subaward records. Find the one small machine shop that three nominally redundant primes both depend on. Frame it as **industrial base resilience and sustainment** — not weapons. One engine, two markets is a story judges like, and the reframe costs you a data adapter rather than a rewrite.
...
Delete from the bottom of every Tier-3 list simultaneously. In order of what you sacrifice first: defense tab, second-user login, coverage strip, `DiffMonitor`, `risk_memo`. Do **not** cut the rehearsals to buy feature time — an unrehearsed demo of a better product loses to a rehearsed demo of a worse one.
...
**"What about SecurityScorecard / Bitsight / UpGuard?"** Per-vendor risk *ratings* — is this vendor well-run. Orthogonal to shared-dependency structure. A stack of five A-rated vendors on one provider still has one point of failure.
...
- [Scale reference — `jac start --scale`, K8s, auth, persistence](https://docs.jaseci.org/reference/plugins/jac-scale/)
